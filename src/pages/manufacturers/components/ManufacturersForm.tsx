    ...
    <Box key={display} sx={{ p: theme => theme.spacing(0, 6, 6) }}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={5}>
          <Grid item xs={12}>
            <DropzoneWrapper>
              <FileUploaderMultiple  files={files} setFiles={setFiles} addFiles={addFiles} fileLink={fileLink} setFileLink={setFileLink} />
            </DropzoneWrapper>
          </Grid>
          <Grid item xs={12}>
      ...
      <DrawerActions handleClose={handleCloseFn} addDisabled={addFiles.isPending} />
      </form>
    </Box>
  )
}
